...
export default function Card({ lecturer, index }: { lecturer: Lecturer_Card, index: number }) {
  ...
  return (
    <Link href={"/lecturer/" + lecturer.uuid}>
      <div ref={ref} className={"grid grid-cols-1 items-center h-96 bg-jet/15 dark:bg-jet rounded-xl p-8 border-2 border-jet dark:border-white overflow-hidden" + (isVisible ? " opacity-100 animate-fade-up animate-ease-in-out animate-alternate " + (isOdd(index) ? "animate-delay-[100ms]" : "") : " opacity-0")}>
